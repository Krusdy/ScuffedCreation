...
class CaptureApp(ctk.CTk):
    ...
    def load_config(self):
        ...
        self.ffmpeg_path_val = self.config.get("Settings", "ffmpeg_path", fallback="ffmpeg.exe")
    ...
    def save_config(self, *args):
        ...
        self.config["Settings"]["ffmpeg_path"] = self.ffmpeg_entry.get()
    ...
    def setup_ui(self):
        ...
        ffmpeg_frame = ctk.CTkFrame(settings_frame, fg_color="transparent")
        ffmpeg_frame.pack(fill="x", padx=10, pady=8)
        ctk.CTkLabel(ffmpeg_frame, text="FFmpeg Path", font=ctk.CTkFont(size=13)).pack(side="left")
        self.btn_ffmpeg_browse = ctk.CTkButton(ffmpeg_frame, text="Browse", width=60, height=28, fg_color="#34495E", hover_color="#2C3E50", command=self.browse_ffmpeg_path)
        self.btn_ffmpeg_browse.pack(side="right", padx=(5, 0))
        self.ffmpeg_entry = ctk.CTkEntry(ffmpeg_frame, height=28)
        self.ffmpeg_entry.insert(0, self.ffmpeg_path_val)
        self.ffmpeg_entry.pack(side="right", fill="x", expand=True)
        self.ffmpeg_entry.bind("<KeyRelease>", self.save_config)
    ...
    def browse_ffmpeg_path(self):
        file = filedialog.askopenfilename(initialdir="/", filetypes=[("Executable", "*.exe"), ("All Files", "*.*")])
        if file:
        ...
        def writer():
            while True:
                ...
                if item is None:
                    break
                idx, frame_data = item
                path = os.path.join(session_dir, f"image_{idx:03d}.{img_format}")
                cv2.imwrite(path, frame_data, flags)
                frame_q.task_done()
